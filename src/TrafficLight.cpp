...
T MessageQueue<T>::receive()
{
    ...
    // The received object should then be returned by the receive function. 
  	...
  	_cVariable.wait(lock,[this]{ return !_queue.empty();});
...
void MessageQueue<T>::send(T &&msg)
{
    ...
    // access the queue section
  	std::lock_guard<std::mutex> lock(_mutex); // lock is released when myLock goes out of scope
...
void TrafficLight::cycleThroughPhases()
{
  ...
  // generate random amount of time
    ...
    std::mt19937 eng(rd());
    std::uniform_int_distribution<> distr(4000, 6000); // between 4 seconds and 6 seconds
  ...
  while(1)
  {
    ...
    if (duration >= cycle_duration)
    {
      ...
      _msgqueue.send(std::move(_currentPhase)); // update msgqueue
      cycle_duration = distr(eng); // get random duration 
